...
export default function Home() {
  ...
  return <main className="lab-shell">
    ...
    <section className="lab-main">
      ...
      <section className="advanced-panel"><div className="advanced-heading"><div className="eyebrow">ADVANCED PARAMETERS / CASE 01</div><h2>Define the physical test conditions.</h2><p>These values will become solver inputs after geometry and mesh preparation.</p></div><div className="param-group"><div className="param-group-title"><Wind size={14} /> Environment</div><label>Air temperature <strong>{temperature} °C</strong><input type="range" min="-10" max="45" value={temperature} onChange={(e) => setTemperature(Number(e.target.value))} /></label><label>Static pressure <strong>{pressure} hPa</strong><input type="range" min="950" max="1050" value={pressure} onChange={(e) => setPressure(Number(e.target.value))} /></label></div><div className="param-group"><div className="param-group-title"><Activity size={14} /> Mass model</div><label>Rider mass <strong>{riderMass} kg</strong><input type="range" min="45" max="120" value={riderMass} onChange={(e) => setRiderMass(Number(e.target.value))} /></label><label>Bicycle mass <strong>{bikeMass.toFixed(1)} kg</strong><input type="range" min="6" max="18" step="0.1" value={bikeMass} onChange={(e) => setBikeMass(Number(e.target.value))} /></label></div><div className="solver-readiness"><div className="param-group-title"><Gauge size={14} /> Solver workflow</div><div className="solver-steps"><span className="active">01 Setup</span><span>02 Mesh</span><span>03 Solve</span><span>04 Review</span></div><div className="readiness-line"><span className="status-dot" /> Geometry preview only <strong>Mesh not generated</strong></div></div></section><section className="geometry-panel"><div className="geometry-heading"><div className="eyebrow">GEOMETRY SPECIFICATION / R-01</div><h2>Reference dimensions for meshing.</h2><p>The current scene is a simplified concept. These values define its scale and coordinate contract before CAD or mesh export.</p></div><div className="geometry-grid"><label>Wheelbase <strong>{wheelbase.toFixed(2)} m</strong><input type="range" min="0.90" max="1.20" step="0.01" value={wheelbase} onChange={(e) => setWheelbase(Number(e.target.value))} /></label><label>Wheel diameter <strong>{wheelDiameter.toFixed(2)} m</strong><input type="range" min="0.65" max="0.75" step="0.01" value={wheelDiameter} onChange={(e) => setWheelDiameter(Number(e.target.value))} /></label><label>Rider height <strong>{riderHeight.toFixed(2)} m</strong><input type="range" min="1.55" max="2.05" step="0.01" value={riderHeight} onChange={(e) => setRiderHeight(Number(e.target.value))} /></label></div><div className="geometry-status"><span className="eyebrow">MESH CONTRACT</span><strong>{geometryStatus}</strong><span className="readiness-copy">Scale locked · surfaces simplified · watertight check pending</span><button className="export-button" onClick={() => setGeometryStatus("Spec ready / mesh pending")}>{geometryStatus === "Preview only" ? "Prepare solver export" : "Spec prepared"}</button></div></section><footer className="lab-footer"><span><Gauge size={14} /> Solver: preview field / no CFD</span><span>Air density {airDensity} kg/m³</span><span>Last saved just now</span></footer>
    </section>
  </main>;
}
