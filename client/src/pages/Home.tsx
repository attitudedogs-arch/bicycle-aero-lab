...
type LocalAsset = { fileName: string; type: string; size: number; url: string; source?: "project" | "browser" };
const providedBicycleAsset: LocalAsset = { fileName: "bicycle_simple_3d_model_glb.glb", type: "GLB / project asset", size: 208432, url: "/manus-storage/bicycle_simple_3d_model_glb_335949ac.glb", source: "project" };
...
function addBike(scene: THREE.Scene, subjectModel: string, customPart: string, riderPreset: string, qualityMode: QualityMode) {
  ...
  const bikeFrame = new THREE.Group(); bikeFrame.name = "procedural-bicycle"; bike.add(bikeFrame);
  const riderGroup = new THREE.Group(); riderGroup.name = "procedural-rider"; bike.add(riderGroup);
  ...
  const addMesh = <T extends THREE.Object3D>(mesh: T, parent: THREE.Object3D = bikeFrame, cast = true) => { mesh.castShadow = cast; mesh.receiveShadow = true; parent.add(mesh); return mesh; };
  const tube = (a: THREE.Vector3, b: THREE.Vector3, radius = 0.05, material = carbon, parent: THREE.Object3D = bikeFrame) => {
  ...
  const wheel = (x: number, deep = false) => {
    const wheelGroup = new THREE.Group(); wheelGroup.position.set(x, 0.6, 0); bikeFrame.add(wheelGroup);
  ...
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.22, 0.48, detail.riderSegments, detail.riderSegments * 2), kit); torso.rotation.z = upright ? -0.34 : -0.82; torso.position.copy(shoulder).add(hip).multiplyScalar(0.5); addRiderMesh(torso);
  const neck = new THREE.Mesh(new THREE.CapsuleGeometry(0.075, 0.08, 5, 8), skin); neck.position.set(0.39, headPos.y - 0.13, 0); addRiderMesh(neck);
  const headMesh = new THREE.Mesh(new THREE.SphereGeometry(0.145, detail.riderSegments * 2, detail.riderSegments), skin); headMesh.position.copy(headPos); addRiderMesh(headMesh);
  const helmet = new THREE.Mesh(new THREE.SphereGeometry(0.17, detail.riderSegments * 2, detail.riderSegments, 0, Math.PI * 2, 0, Math.PI / 1.7), carbonEdge); helmet.position.set(headPos.x, headPos.y + 0.04, 0); addRiderMesh(helmet);
  const visorMesh = new THREE.Mesh(new THREE.CapsuleGeometry(0.035, 0.11, 4, 8), visor); visorMesh.rotation.z = Math.PI / 2; visorMesh.position.set(headPos.x + 0.11, headPos.y + 0.015, -0.02); addRiderMesh(visorMesh);
  const elbowNear = new THREE.Vector3(0.24, shoulder.y - 0.1, -0.12), handNear = new THREE.Vector3(0.66, 1.22, -0.16); riderTube(shoulder, elbowNear, 0.065, kit); riderTube(elbowNear, handNear, 0.052, skin); const elbowFar = new THREE.Vector3(0.24, shoulder.y - 0.1, 0.12), handFar = new THREE.Vector3(0.66, 1.22, 0.16); riderTube(shoulder, elbowFar, 0.065, kit); riderTube(elbowFar, handFar, 0.052, skin);
  const kneeNear = new THREE.Vector3(-0.28, 1.05, -0.1), ankleNear = new THREE.Vector3(-0.16, 0.69, -0.1); riderTube(hip, kneeNear, 0.075, kitDark); riderTube(kneeNear, ankleNear, 0.054, skin); const kneeFar = new THREE.Vector3(-0.2, 1.02, 0.1), ankleFar = new THREE.Vector3(0.02, 0.66, 0.1); riderTube(hip, kneeFar, 0.075, kitDark); riderTube(kneeFar, ankleFar, 0.054, skin);
  const shoeNear = new THREE.Mesh(new THREE.CapsuleGeometry(0.045, 0.18, 5, 8), carbonEdge); shoeNear.rotation.z = Math.PI / 2; shoeNear.position.set(-0.04, 0.65, -0.1); addRiderMesh(shoeNear); const shoeFar = shoeNear.clone(); shoeFar.position.z = 0.1; addRiderMesh(shoeFar);
  ...
  useEffect(() => {
    ...
    const loadRoleAsset = (role: AssetRole, url?: string) => {
      ...
      loader.load(url, (gltf) => {
        ...
        const fitScale = role === "custom" ? 1.2 / maxDimension : role === "bicycle" ? 3.2 / maxDimension : 2.2 / maxDimension;
        ...
        if (role === "bicycle") { const proceduralBicycle = bike.getObjectByName("procedural-bicycle"); if (proceduralBicycle) proceduralBicycle.visible = false; }
      }, undefined, () => { if (role === "bicycle") { const proceduralBicycle = bike.getObjectByName("procedural-bicycle"); if (proceduralBicycle) proceduralBicycle.visible = true; } });
    };
...
export default function Home() {
  ...
  const [localAsset, setLocalAsset] = useState<LocalAsset | null>(providedBicycleAsset);
  ...
  useEffect(() => () => { [localAsset, riderAsset, customAsset].forEach((asset) => { if (asset?.url && asset.source !== "project") URL.revokeObjectURL(asset.url); }); }, [localAsset, riderAsset, customAsset]);
  const handleRoleAssetInput = (role: AssetRole, file?: File) => {
    ...
    setAssetInputError("");
    const nextAsset = { fileName: file.name, type: file.name.toLowerCase().endsWith(".glb") ? "GLB" : "GLTF", size: file.size, url: URL.createObjectURL(file), source: "browser" as const };
  ...
  const restoreCase = (savedCase: SavedCase) => {
    ...
    setLocalAsset(providedBicycleAsset);
    setRiderAsset(null);
    setCustomAsset(null);
