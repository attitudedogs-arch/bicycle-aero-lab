...
function Scene({ windAngle, airflow, resetSignal, subjectModel, customPart, riderPreset, surfaceType, assetUrls }: { windAngle: number; airflow: boolean; resetSignal: number; subjectModel: string; customPart: string; riderPreset: string; surfaceType: string; assetUrls: Partial<Record<AssetRole, string>> }) {
  ...
  useEffect(() => {
    ...
    const loadRoleAsset = (role: AssetRole, url?: string) => {
      ...
      loader.load(url, (gltf) => {
        ...
        loadedAsset.position.set(-center.x * fitScale, -box.min.y * fitScale + (role === "rider" ? 0.05 : 0), -center.z * fitScale + (role === "custom" ? 0.18 : 0));
  ...
  }, [windAngle, airflow, resetSignal, subjectModel, customPart, riderPreset, surfaceType, assetUrls.bicycle, assetUrls.rider, assetUrls.custom]);
