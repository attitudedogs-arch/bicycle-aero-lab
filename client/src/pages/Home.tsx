...
function Scene({ windAngle, airflow, resetSignal, subjectModel, customPart, riderPreset, surfaceType }: { windAngle: number; airflow: boolean; resetSignal: number; subjectModel: string; customPart: string; riderPreset: string; surfaceType: string }) {
  ...
  useEffect(() => {
    ...
    const bike = addBike(scene, subjectModel, customPart, riderPreset); const air = addAirflow(scene, airflow ? 1 : 0.42);
  ...
  }, [windAngle, airflow, resetSignal, subjectModel, customPart, riderPreset, surfaceType]);
